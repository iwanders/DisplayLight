...
public:
   ...
   */
  Box findBorders(const Image& screen, size_t bisects_per_side=4) const;
   ...
   */
  std::vector<BoxedSamples> makeBoxedSamplePoints(const size_t dist_between_samples, const Box& bounds);
   ...
   */
  void sampleBoxedSamples(const Image& screen, const Box& bounds, const std::vector<BoxedSamples>& boxed_samples, std::vector<RGB>& canvas);
